...
from tf_datasets.core.download import download_google_drive
from tf_datasets.core.download import download_http
...
class wider_face(BaseDataset):
  ...
  annotation_data_url = 'http://mmlab.ie.cuhk.edu.hk/projects/WIDERFace/support/bbx_annotation/wider_face_split.zip'  # noqa
    ...
    for file_id, filename, file_size in [
      ...
      if not os.path.exists(output_path):
        download_google_drive(
            file_id, output_path, self.data_url, file_size)

    output_path = os.path.join(self.download_dir, 'wider_face_split.zip')
    if not os.path.exists(output_path):
      download_http(self.annotation_data_url, output_path)
  ...
  def _get_data_points(self):
    ...
    filenames_to_bboxes = {}
    filenames_to_bboxes.update(_load_label_file(os.path.join(
